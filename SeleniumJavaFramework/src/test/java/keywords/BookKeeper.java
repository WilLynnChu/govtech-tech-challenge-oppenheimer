...
public class BookKeeper extends BookKeeperObject{
	...
	public static void verifyDBFileTable(String test_start_time) {
		...
		String file_type = SQLHelper.getFirstRowKeyValueFromDBResult(sql_file_response, "file_type");
		String file_status = SQLHelper.getFirstRowKeyValueFromDBResult(sql_file_response, "file_status");
		Assert.assertEquals("TAX_RELIEF", file_type, "file_type: " + file_type + " is not equal to TAX_RELIEF!");
		Assert.assertEquals("COMPLETED", file_status, "file_status: " + file_status + " is not equal to COMPLETED!");
	}
}
